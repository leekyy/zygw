...
class RuleManager
{
    ...
    {
        $xjs = Rules::orderby('id', 'desc')->where('status', '=', '1');
    ...
    {
        $xjs = Rules::orderby('id', 'desc')->paginate(Utils::PAGE_SIZE);
        return $xjs;
    }
    /*
     * 获取全部生效积分兑换规则信息
    ...
    {
        $steps = RuleTWStep::where('f_id', '=', $xj_id)->orderby('seq', 'asc')->get();
        return $steps;
    }
    /*
     * 根据f_id和f_table获取积分兑换规则步骤信息
     *
     * By TerryQi
     *
     * 2017-12-23
    ...
    {
        $steps = RuleTWStep::where('f_table', '=', $f_table)->where('f_id', '=', $f_id)->get();
        return $steps;
    }
    /*
     * 根据f_id和f_table删除全部积分兑换规则步骤
     *
     * By TerryQi
     *
     * 2107-12-23
    ...
    {
        $steps = RuleTWStep::where('f_table', '=', $f_table)->where('f_id', '=', $f_id)->delete();
    }
    /*
     * 积分兑换规则展示数加1
     *
    ...
    {
        $tw_step = RuleTWStep::where('id', '=', $id)->first();
        return $tw_step;
    }
    /*
     * 设置积分兑换规则相关信息
     ...
     *
     */
    public static function setRule($xj, $data)
    {
        ...
        if (array_key_exists('admin_id', $data)) {
            $xj->doctor_id = array_get($data, 'admin_id');
        }
     ...
     *
     */
    public static function setRuleStep($tw_step, $data)
    {

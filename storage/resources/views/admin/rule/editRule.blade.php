        ...
        <div class="modal-dialog">
            <div class="modal-content message_align">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">管理积分兑换规则信息</h4>
                </div>
    ...
    <script type="application/javascript">
        ...
        $(document).ready(function () {
            ...
            if (judgeIsNullStr(xj_id)) {
                ...
                getRuleInfoById("{{URL::asset('')}}", param, function (ret, err) {
                    //提示保存成功
                    if (ret.result == true) {
                        xjInfo = ret.ret;
                        loadHtml();
        ...
        function clickSave() {
            ...
            if (xjInfo.steps.length <= 0) {
                $("#tipModalBody").html('<p>请录入积分兑换规则图文信息</p>');
                $("#tipModal").modal('show');
                return;
            }
            //进行排序
            ...
            //调用接口进行编辑
            editRule("{{URL::asset('')}}", JSON.stringify(xjInfo), function (ret, err) {
                //提示保存成功
                if (ret.result == true) {
                    $("#tipModalBody").html('<p>积分兑换规则信息保存成功</p>');
                    $("#tipModal").modal('show');
                    xjInfo = ret.ret;
                    loadHtml();
                } else {
                    $("#tipModalBody").html("<p>积分兑换规则信息保存失败，请联系<span class='text-info'>管理员处理</span></p>");
                    $("#tipModal").modal('show');
                }
            })
        }

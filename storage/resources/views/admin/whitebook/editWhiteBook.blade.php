        ...
        <div class="modal-dialog">
            <div class="modal-content message_align">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">管理行业白皮书信息</h4>
                </div>
    ...
    <script type="application/javascript">
        ...
        $(document).ready(function () {
            ...
            if (judgeIsNullStr(xj_id)) {
                ...
                getWhiteBookInfoById("{{URL::asset('')}}", param, function (ret, err) {
                    //提示保存成功
                    if (ret.result == true) {
                        xjInfo = ret.ret;
                        loadHtml();
        ...
        function clickSave() {
            ...
            if (xjInfo.steps.length <= 0) {
                $("#tipModalBody").html('<p>请录入行业白皮书图文信息</p>');
                $("#tipModal").modal('show');
                return;
            }
            //进行排序
            ...
            //调用接口进行编辑
            editWhiteBook("{{URL::asset('')}}", JSON.stringify(xjInfo), function (ret, err) {
                //提示保存成功
                if (ret.result == true) {
                    $("#tipModalBody").html('<p>行业白皮书信息保存成功</p>');
                    $("#tipModal").modal('show');
                    xjInfo = ret.ret;
                    loadHtml();
                } else {
                    $("#tipModalBody").html("<p>行业白皮书保存失败，请联系<span class='text-info'>管理员处理</span></p>");
                    $("#tipModal").modal('show');
                }
            })
        }
